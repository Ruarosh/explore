...
template <typename Real>
bool MatrixBase<Real>::PreAddMatMat(const Real alpha,
                                    const MatrixBase<Real>& A, MatrixTransposeType transA,
                                    const MatrixBase<Real>& B, MatrixTransposeType transB,
                                    const Real beta,
                                    bool to_dequantize) {
  Matrix<Real> tmp_A(A), tmp_B(B);

  tmp_A.Quantized();
  tmp_B.Quantized();

  Real lhs_min = tmp_A.quant_params_.range_min_val;
  Real lhs_max = tmp_A.quant_params_.range_max_val;
  Real rhs_min = tmp_B.quant_params_.range_min_val;
  Real rhs_max = tmp_B.quant_params_.range_max_val;

  // quant matmul would crash!
  if (lhs_min == lhs_max || rhs_min == rhs_max) {
    tmp_A.Dequantized();
    tmp_B.Dequantized();

    cblas_Xgemm(alpha, transA, tmp_A.data_, tmp_A.num_rows_, tmp_A.num_cols_, tmp_A.stride_,
...
void MatrixBase<Real>::AddMatMat(const Real alpha,
  ...
  // both type are real type, no quantize operation here!
  if(A.data_type_ == B.data_type_ && REAL_DATA == A.data_type_) {
    // // real type
    AddMatMatReal(alpha, A, transA, B, transB, beta);
    return;
  }

  if(A.data_type_ == B.data_type_ && QUANT_DATA == A.data_type_) {
    AddMatMatL8R8ToRst32_2(alpha, A, transA, B, transB, beta, to_dequantize);
    return;
  }

  // pre-check matrix data
  // if(PreAddMatMat(alpha, A, transA, B, transB, beta, to_dequantize)) {
  //   return;
  // }

   // // TODO: int8 * int8 ==> int8, which is faster theoretically.
  // AddMatMatL8R8ToRst8(alpha, A, transA, B, transB, beta, to_dequantize);

  // // int8 * int8 ==> int32
  AddMatMatL8R8ToRst32(alpha, A, transA, B, transB, beta, to_dequantize);
}

template <typename Real>
void MatrixBase<Real>::AddMatMatReal(const Real alpha,
                                 const MatrixBase<Real>& A, MatrixTransposeType transA,
                                 const MatrixBase<Real>& B, MatrixTransposeType transB,
                                 const Real beta) {
...
template <typename Real>
void MatrixBase<Real>::AddMatMatL8R8ToRst8(const Real alpha,
                                 const MatrixBase<Real>& A, MatrixTransposeType transA,
                                 const MatrixBase<Real>& B, MatrixTransposeType transB,
                                 const Real beta,
                                 bool to_dequantize) {
  Matrix<Real> tmp_A(A);
  tmp_A.Quantized();
  int32_t depth = A.NumRows() + A.NumCols() - num_rows_;
  int lda = (kTrans == transA) ? A.NumRows() : A.NumCols();
  int ldb = (kNoTrans == transB) ? B.NumCols() : B.NumRows();
  gemmlowp::GemmContext context;
#ifdef HAVE_OPENBLAS
  context.set_max_num_threads(::openblas_get_num_threads());
#endif
  // context.set_l1_bytes_to_use(32768/4);   //  32768  = 1024 * 32
  // context.set_l2_bytes_to_use(262144/4); //  262144 = 1024 * 256
  QuantizationParams mul_result;
  OutputPipelineType output_pipeline;
  std::vector<BaseQuantType> uint8_mul_result;
  uint8_mul_result.reserve(num_rows_ * num_cols_);

  OutputPipeline(tmp_A.quant_params_, B.quant_params_, mul_result, output_pipeline);
  GemmlowpMultiplyL8R8ToRst8(&context, (BaseQuantType *)tmp_A.Data(), transA,
                   (BaseQuantType *)B.Data(), transB, depth,
                   tmp_A.quant_params_.zero_point, B.quant_params_.zero_point,
                   lda, ldb, ldb, uint8_mul_result.data(), output_pipeline);
  this->Quantized();
  QuantizedAdd(uint8_mul_result.data(), to_dequantize, &mul_result);
}


template <typename Real>
void MatrixBase<Real>::AddMatMatL8R8ToRst32_2(const Real alpha,
                                 const MatrixBase<Real>& A, MatrixTransposeType transA,
                                 const MatrixBase<Real>& B, MatrixTransposeType transB,
                                 const Real beta,
                                 bool to_dequantize) {
...
#ifdef HAVE_OPENBLAS
  context.set_max_num_threads(::openblas_get_num_threads());
  // context.set_l2_rhs_factor(0.72);
#endif
  // context.set_l1_bytes_to_use(32768/4);   //  32768  = 1024 * 32
  // context.set_l2_bytes_to_use(262144/4); //  262144 = 1024 * 256
  QuantizationParams mul_result;
  OutputPipelineType output_pipeline; // for tL8R8ToRst8
  std::vector<int32_t> int32_mul_result;
  int32_mul_result.reserve(num_rows_ * num_cols_);

  OutputPipeline(A.quant_params_, B.quant_params_, mul_result, output_pipeline);
  GemmlowpMultiplyL8R8ToRst32(&context, (BaseQuantType *)A.Data(), transA,
                   (BaseQuantType *)B.Data(), transB, depth,
                    A.quant_params_.zero_point, B.quant_params_.zero_point,
                   lda, ldb, ldb, (BaseQuantType *)int32_mul_result.data());

  this->DequantizedAdd(int32_mul_result.data(), mul_result);
  if(!to_dequantize) {
    KALDI_LOG << "QUANTIZE RESULT ONCE";
    this->Quantized();
  }
}
...
template <typename Real>
void MatrixBase<Real>::AddMatMatL8R8ToRst32(const Real alpha,
                                 const MatrixBase<Real>& A, MatrixTransposeType transA,
                                 const MatrixBase<Real>& B, MatrixTransposeType transB,
                                 const Real beta,
                                 bool to_dequantize) {
  ...
  OutputPipeline(tmp_A.quant_params_, B.quant_params_, mul_result, output_pipeline);
  GemmlowpMultiplyL8R8ToRst32(&context, (BaseQuantType *)tmp_A.Data(), transA,
                   (BaseQuantType *)B.Data(), transB, depth,
                   tmp_A.quant_params_.zero_point, B.quant_params_.zero_point,
                   lda, ldb, ldb, (BaseQuantType *)int32_mul_result.data());
...
/**
 * for Write function, which has const qualifier
 */
template <typename Real>
void MatrixBase<Real>::Quantized(const MatrixBase<Real>& src_mat,
                                 std::vector<BaseQuantType>& dst_mat,
                                 const Real min_val, const Real max_val,
                                 int& offset) const {
  QuantizationParams quant_param;
  ChooseQuantizationParams(min_val, max_val, &quant_param);
  double quant_scale = quant_param.quant_scale;
  src_mat.RealTensorToQuantizedInPlace(dst_mat.data(), quant_param);
  offset = RealToQuantizedOffset<BaseQuantType>(0.0f, min_val, quant_scale);
}

template <typename Real> void MatrixBase<Real>::Quantized() {
  if (this->data_type_ == QUANT_DATA) {
    return;
  }
  ...
  quant_params_.range_min_val = min_val;
  quant_params_.range_max_val = max_val;

  ChooseQuantizationParams(min_val, max_val, &quant_params_);
  RealTensorToQuantizedInPlace((BaseQuantType *)this->data_, quant_params_);

  this->data_type_ = QUANT_DATA;
}

template <typename Real>
template <typename InputType>
double MatrixBase<Real>::QuantizeScaleAdd(Real range_min,
                                          Real range_max) const {
  int64_t lowest_quantized = 0;
  if (std::is_same<InputType, int32_t>::value) {
    lowest_quantized = INT32_MIN;
  }
  ...
  const int number_of_bits = sizeof(InputType) * 8;
...
template <typename Real>
template <typename InputType>
BaseQuantType MatrixBase<Real>::RealToQuantized(Real input, Real range_min,
                                                double scale) const {
  int64_t quantized = RealToQuantizedOffset<InputType>(input, range_min, scale);
  const int64_t lowest_quantized = static_cast<int64_t>(UINT8_MIN);
  const int64_t highest_quantized = static_cast<int64_t>(UINT8_MAX);
  quantized = std::max(quantized, lowest_quantized);
  quantized = std::min(quantized, highest_quantized);

  return static_cast<BaseQuantType>(static_cast<int32_t>(quantized));
}

// real_size is always larger than quant_size
template <typename Real>
void MatrixBase<Real>::RealTensorToQuantizedInPlace(
    BaseQuantType *dst, QuantizationParams &quant_param) const {
  auto scale = quant_param.quant_scale;
  auto zero_point = quant_param.zero_point;

  for (int i = 0; i < num_rows_; ++i) {
    for (int j = 0; j < num_cols_; ++j) {
      float trans_val =
          zero_point + *(this->data_ + i * this->stride_ + j) / scale;
      const Real clamped_val = std::max(0.f, std::min(255.f, trans_val));
      dst[i * num_cols_ + j] =
          static_cast<std::uint8_t>(std::round(clamped_val));
    }
  }
}
...
template <typename Real>
template <typename InputType>
int64_t MatrixBase<Real>::RealToQuantizedOffset(Real input, Real range_min,
                                                double range_scale) const {
  int64_t lowest_quantized = static_cast<double>(UINT8_MIN);
  if (std::is_same<InputType, int32_t>::value) {
    lowest_quantized = INT32_MIN;
  }
  if (0.0f == range_scale) {
...
template <typename Real> void MatrixBase<Real>::SetMatrixType(DataTypeT type) {
  data_type_ = type;
}

template <typename Real>
void MatrixBase<Real>::DequantizedAdd(int32_t* data, const QuantizationParams& params) {
  auto min_val = params.range_min_val;
  auto quant_scale = params.quant_scale;
  Real *real_ptr = (Real *)this->data_;
  int32_t *int32_data = data;

  for (int i = 0; i < this->num_rows_; ++i) {
    for (int j = 0; j < this->num_cols_; ++j) {
      *(real_ptr + i * this->stride_ + j) += QuantizedToReal<int32_t>(
          *(int32_data + i * this->num_cols_ + j), min_val, quant_scale);
    }
  }

  this->data_type_ = REAL_DATA;
}

template <typename Real> void MatrixBase<Real>::Dequantized() {
  auto min_val = this->quant_params_.range_min_val;
  if (QUANT_DATA != this->data_type_) {
    return;
  }

  auto quant_scale = this->quant_params_.quant_scale;

  std::vector<Real> tmp_real_data;
  const MatrixIndexT size = num_rows_ * num_cols_;
  tmp_real_data.reserve(size);

  auto *real_ptr = tmp_real_data.data();
  BaseQuantType *quant_data = (BaseQuantType *)data_;

  for (int i = 0; i < size; ++i) {
    *(real_ptr + i) =
        QuantizedToReal<BaseQuantType>(*(quant_data + i), min_val, quant_scale);
  }

  std::memcpy(data_, real_ptr, sizeof(Real) * size);
...
template <typename Real>
void MatrixBase<Real>::QuantizeMultiplierSmallerThanOne(float real_multiplier, int32_t* quantized_multiplier, int* right_shift) const {
...
template <typename Real>
void MatrixBase<Real>::OutputPipeline(
    const QuantizationParams& lhs_qparams,
    const QuantizationParams& rhs_qparams, QuantizationParams &mul_result,
    OutputPipelineType& output_pipeline) const {
  this->QuantizationRangeForMultiplication(
      lhs_qparams.range_min_val, lhs_qparams.range_max_val,
      rhs_qparams.range_min_val, rhs_qparams.range_max_val,
      mul_result.range_min_val,  mul_result.range_max_val);

  mul_result.quant_scale = DequantizeScale<int32_t>(mul_result.range_min_val, mul_result.range_max_val);
/*
  // TODO: for L8R8Rst8, need actual output range for quant output
  // mul_result.range_min_val = -300.0f;
  // mul_result.range_max_val = 500.0f;

  int right_shift;
  int quantized_multiplier;

  this->ChooseQuantizationParams(mul_result.range_min_val,
                                 mul_result.range_max_val, &mul_result);

  const int result_offset = mul_result.zero_point;
  float real_multiplier = lhs_qparams->quant_scale * rhs_qparams->quant_scale /
                          mul_result.quant_scale;
  ...
  output_pipeline = std::make_tuple(quantize_down_stage, saturating_cast_stage);
*/
}

template <typename Real>
void MatrixBase<Real>::GemmlowpMultiplyL8R8ToRst8(
    gemmlowp::GemmContext *context, const BaseQuantType *a_data,
    const MatrixTransposeType A_order, const BaseQuantType *b_data,
    const MatrixTransposeType B_order, int depth, int offset_a, int offset_b,
    int lda, int ldb, int ldc, BaseQuantType *output_data,
    OutputPipelineType &output_pipeline) {

  const BaseQuantType *a_data_as_quant_type = a_data;
  const BaseQuantType *b_data_as_quant_type = b_data;

  gemmlowp::MatrixMap<BaseQuantType, gemmlowp::MapOrder::RowMajor> result_mat(output_data, num_rows_, num_cols_, num_cols_);
  if (kTrans == A_order && kTrans == B_order) {
    gemmlowp::MatrixMap<const BaseQuantType, gemmlowp::MapOrder::ColMajor> lhs(a_data_as_quant_type, num_rows_, depth, num_rows_);
    gemmlowp::MatrixMap<const BaseQuantType, gemmlowp::MapOrder::ColMajor> rhs(b_data_as_quant_type, depth, num_cols_, depth);
    gemmlowp::GemmWithOutputPipeline<BaseQuantType, BaseQuantType, gemmlowp::DefaultL8R8BitDepthParams>(context, lhs, rhs, &result_mat, -offset_a, -offset_b, output_pipeline);
  } else if (kTrans == A_order && kTrans != B_order) {
    gemmlowp::MatrixMap<const BaseQuantType, gemmlowp::MapOrder::ColMajor> lhs(a_data_as_quant_type, num_rows_, depth, num_rows_);
    gemmlowp::MatrixMap<const BaseQuantType, gemmlowp::MapOrder::RowMajor> rhs(b_data_as_quant_type, depth, num_cols_, num_cols_);
    gemmlowp::GemmWithOutputPipeline<BaseQuantType, BaseQuantType, gemmlowp::DefaultL8R8BitDepthParams>(context, lhs, rhs, &result_mat, -offset_a, -offset_b, output_pipeline);
  } else if (kTrans != A_order && kTrans == B_order) {
    gemmlowp::MatrixMap<const BaseQuantType, gemmlowp::MapOrder::RowMajor> lhs(a_data_as_quant_type, num_rows_, depth, depth);
    gemmlowp::MatrixMap<const BaseQuantType, gemmlowp::MapOrder::ColMajor> rhs(b_data_as_quant_type, depth, num_cols_, depth);
    gemmlowp::GemmWithOutputPipeline<BaseQuantType, BaseQuantType, gemmlowp::DefaultL8R8BitDepthParams>(context, lhs, rhs, &result_mat, -offset_a, -offset_b, output_pipeline);
  } else {
    gemmlowp::MatrixMap<const BaseQuantType, gemmlowp::MapOrder::RowMajor> lhs(a_data_as_quant_type, num_rows_, depth, depth);
...
template <typename Real>
void MatrixBase<Real>::GemmlowpMultiplyL8R8ToRst32(
    gemmlowp::GemmContext *context, const BaseQuantType *a_data,
    const MatrixTransposeType A_order, const BaseQuantType *b_data,
    const MatrixTransposeType B_order, int depth, int offset_a, int offset_b,
    int lda, int ldb, int ldc, BaseQuantType *output_data) {
  ...
  gemmlowp::MatrixMap<int32_t, gemmlowp::MapOrder::RowMajor> result_mat((int32_t *)output_data, num_rows_, num_cols_, num_cols_);
  if (kTrans == A_order && kTrans == B_order) {
    gemmlowp::MatrixMap<const BaseQuantType, gemmlowp::MapOrder::ColMajor> lhs(a_data_as_quant_type, num_rows_, depth, num_rows_);
    gemmlowp::MatrixMap<const BaseQuantType, gemmlowp::MapOrder::ColMajor> rhs(b_data_as_quant_type, depth, num_cols_, depth);
    gemmlowp::GemmWithOutputPipeline<BaseQuantType, int32_t, gemmlowp::DefaultL8R8BitDepthParams>(context, lhs, rhs, &result_mat, -offset_a, -offset_b, empty_pipeline);
  } else if (kTrans == A_order && kTrans != B_order) {
    gemmlowp::MatrixMap<const BaseQuantType, gemmlowp::MapOrder::ColMajor> lhs(a_data_as_quant_type, num_rows_, depth, num_rows_);
    gemmlowp::MatrixMap<const BaseQuantType, gemmlowp::MapOrder::RowMajor> rhs(b_data_as_quant_type, depth, num_cols_, num_cols_);
    gemmlowp::GemmWithOutputPipeline<BaseQuantType, int32_t, gemmlowp::DefaultL8R8BitDepthParams>(context, lhs, rhs, &result_mat, -offset_a, -offset_b, empty_pipeline);
...
template <typename Real>
void Matrix<Real>::ReadQuantizedData(std::istream &is, bool binary) {
  ...
  is.read(reinterpret_cast<char *>(&min_val), sizeof(BaseFloat));
  is.read(reinterpret_cast<char *>(&max_val), sizeof(BaseFloat));
  ...
  this->quant_params_.range_min_val = min_val;
  this->quant_params_.range_max_val = max_val;

  if (this->Stride() == this->NumCols() && rows * cols != 0) {
    is.read(reinterpret_cast<char *>(this->data_), sizeof(BaseQuantType) * rows * cols);
    if (is.fail()) { KALDI_ERR << "encounter bad case!!!"; }
  } else {
    for (MatrixIndexT i = 0; i < (MatrixIndexT)rows; i++) {
      is.read(reinterpret_cast<char *>(this->data_ + i * this->num_cols_), sizeof(BaseQuantType) * cols);
      if (is.fail()) { KALDI_ERR << "encounter bad case!!!"; }
    }
  }

  MatrixBase<Real>::ChooseQuantizationParams(this->quant_params_.range_min_val,
                                             this->quant_params_.range_max_val,
                                             &this->quant_params_);
...
template <typename Real>
void MatrixBase<Real>::ChooseQuantizationParams(
    const BaseFloat min_val, const BaseFloat max_val,
    QuantizationParams* out_params) const {
  if (min_val == max_val) {
    out_params->quant_scale = 0.0f;
    out_params->zero_point = 0;
  } else {
    auto min = std::min(0.f, min_val);
    auto max = std::max(0.f, max_val);
    ...
    out_params->quant_scale = scale;
    out_params->zero_point = nudged_zero_point;
  }
}

template <typename Real>
Real MatrixBase<Real>::RealForOneQuantizedLevel(Real range_min,
                                                Real range_max) const {
  const int64_t highest = static_cast<int64_t>(UINT8_MAX);
  const int64_t lowest = static_cast<int64_t>(UINT8_MIN);
  return ((range_max - range_min) / (highest - lowest));
}

template <typename Real>
void MatrixBase<Real>::QuantizationRangeForMultiplication(
    Real min_a, Real max_a, Real min_b, Real max_b, BaseFloat &min_output,
    BaseFloat &max_output) const {
  const Real a_real_for_one_quant_level =
      RealForOneQuantizedLevel(min_a, max_a);
  const Real b_real_for_one_quant_level =
      RealForOneQuantizedLevel(min_b, max_b);

  const int64_t c_highest = static_cast<int64_t>(INT_MAX);
  const int64_t c_lowest = static_cast<int64_t>(INT_MIN);
  const Real c_real_for_one_quant_level =
      a_real_for_one_quant_level * b_real_for_one_quant_level;

  min_output = c_real_for_one_quant_level * c_lowest;
  max_output = c_real_for_one_quant_level * c_highest;
}

template <typename Real>
template <typename InputType>
double MatrixBase<Real>::DequantizeScale(const BaseFloat min_val,
                                         const BaseFloat max_val) const {
  int number_of_bits = sizeof(InputType) * 8;
  const int64_t number_of_steps = static_cast<int64_t>(1) << number_of_bits;
  const double range_adjust = (number_of_steps / (number_of_steps - 1.0));
  const double range = ((max_val - min_val) * range_adjust);
...
template <typename Real>
template <typename InputType>
Real MatrixBase<Real>::QuantizedToReal(InputType input, Real range_min,
                                       double range_scale) {
  if (range_scale == 0.0f) {
    return range_min;
  }

  int64_t lowest_quantized = static_cast<int64_t>(INT_MIN);
  // quant type
  if (1 == sizeof(InputType)) {
    lowest_quantized = UINT8_MIN;
  }

  const double offset_input = static_cast<double>(input) - lowest_quantized;
  const double range_min_rounded =
      round(range_min / static_cast<Real>(range_scale)) *
      static_cast<Real>(range_scale);
...
template <typename Real>
void MatrixBase<Real>::QuantizedTensorInt32ToUint8(const int32_t *input_data,
                                                   bool to_dequantize,
                                                   const BaseFloat min_val,
                                                   const BaseFloat max_val) {
  QuantizationParams out_params;
  ChooseQuantizationParams(min_val, max_val, &out_params);

  auto quant_scale = out_params.quant_scale;
  int32_t dim = num_rows_ * num_cols_;

  std::vector<Real> temp_real_data;
  temp_real_data.reserve(dim);

  auto dequant_scale = DequantizeScale<int32_t>(min_val, max_val);

  for (int i = 0; i < dim; ++i) {
    temp_real_data[i] = QuantizedToReal<int32_t>(*(input_data + i), min_val, dequant_scale);
  }

  memcpy(data_, temp_real_data.data(), dim * sizeof(Real));
  this->data_type_ = REAL_DATA;
  ...
  this->quant_params_.range_min_val = min_val;
  this->quant_params_.range_max_val = max_val;
  this->quant_params_.quant_scale = quant_scale;
  this->quant_params_.zero_point = out_params.zero_point;
}

/**
 * quantized add funcitons
 * A + B means this + input_mat
 **/
template <typename Real>
void MatrixBase<Real>::QuantizedAdd(const BaseQuantType* input_mat,
                                    bool to_dequantize,
                                    const QuantizationParams* quant_param) {
  const Real input_min = quant_param->range_min_val;
  const Real input_max = quant_param->range_max_val;
  const BaseFloat input_scale = quant_param->quant_scale;

  BaseFloat output_min, output_max;
  GetOutputRange(input_min, input_max, &output_min, &output_max);
  auto range_scale = QuantizeScaleAdd<int32_t>(output_min, output_max);

  std::vector<int32_t> output;
  auto dim = num_rows_ * num_cols_;
  output.reserve(dim);

  const BaseFloat vector_0_float = QuantizedToReal<BaseQuantType>(
      0, this->quant_params_.range_min_val, this->quant_params_.quant_scale);
  const BaseFloat vector_1_float = QuantizedToReal<BaseQuantType>(
      1, this->quant_params_.range_min_val, this->quant_params_.quant_scale);
  const int64_t vector_0_int64 =
      RealToQuantizedOffset<int32_t>(vector_0_float, output_min, range_scale);
  const int64_t vector_1_int64 =
      RealToQuantizedOffset<int32_t>(vector_1_float, output_min, range_scale);
  const int32_t vector_mult_int32 = vector_1_int64 - vector_0_int64;

  const BaseFloat tensor_0_float =
      QuantizedToReal<BaseQuantType>(0, input_min, input_scale);
  const BaseFloat tensor_1_float =
      QuantizedToReal<BaseQuantType>(1, input_min, input_scale);
  const int64_t tensor_0_int64 =
      RealToQuantizedOffset<int32_t>(tensor_0_float, output_min, range_scale);
  const int64_t tensor_1_int64 =
      RealToQuantizedOffset<int32_t>(tensor_1_float, output_min, range_scale);

  const int32_t tensor_mult_int32 = tensor_1_int64 - tensor_0_int64;

  const int64_t lowest_quantized = static_cast<int64_t>(INT32_MIN);
  const int64_t highest_quantized = static_cast<int64_t>(INT32_MAX);

  BaseQuantType *qdata = (BaseQuantType *)(data_);

  for (int i = 0; i < dim; ++i) {
    const int64_t vector_value = static_cast<int64_t>(qdata[i]);
    ...
    const int64_t tensor_value = static_cast<int64_t>(input_mat[i]);
  ...
  // int32 --> float  --> uint8
  QuantizedTensorInt32ToUint8(output.data(), to_dequantize, output_min,
                              output_max);
}

template <typename Real>
void MatrixBase<Real>::GetOutputRange(const BaseFloat input_min,
                                      const BaseFloat input_max,
                                      BaseFloat* output_min,
                                      BaseFloat* output_max) {
  const BaseFloat smallest_min =
      std::min(this->quant_params_.range_min_val, input_min);
  const BaseFloat largest_max =
      std::max(this->quant_params_.range_max_val, input_max);
  const BaseFloat biggest_range =
      std::max(std::max(smallest_min, largest_max),
               std::max(-smallest_min, -largest_max));
  const BaseFloat output_range = (biggest_range * (1 << 14));

  *output_max = output_range;
  *output_min = -(*output_max);
}

#ifdef HAVE_BLAS_PRECOPY
template<typename Real>
void MatrixBase<Real>::MatPrecopy(MatrixTransposeType transA,
                                  MatrixTransposeType transB,
                                  const MatrixBase<Real> &B) {
  KALDI_ASSERT(B.num_cols_ == num_cols_ && B.num_rows_ == num_rows_);
  KALDI_ASSERT(&B != this);
  if (num_rows_ == 0 || num_cols_ == 0)
    return;

  cblas_Xgemm_precopy(transA, transB, B.data_, B.num_rows_, B.num_cols_,
                      B.stride_, data_, num_rows_, num_cols_, stride_);
}

template<typename Real>
void MatrixBase<Real>::AddMatMatPrecopy(
    const Real alpha, const MatrixBase<Real> &A, MatrixTransposeType transA,
  ...
  KALDI_ASSERT(&A !=  this && &B != this);
  if (num_rows_ == 0) return;
  cblas_Xgemm_mul(alpha, transA, A.data_, A.num_rows_, A.num_cols_, A.stride_,
              transB, B.data_, B.stride_, beta, data_, num_rows_, num_cols_, stride_);
}
#endif
...
inline void Matrix<Real>::Init(const MatrixIndexT rows,
  ...
  if (NULL != (data = KALDI_MEMALIGN(16, size, &temp))) {
    ...
    MatrixBase<Real>::current_size_ = size;
  } else {
    throw std::bad_alloc();
  }
}
...
#ifdef HAVE_BLAS_PRECOPY
template<typename Real>
inline void Matrix<Real>::InitAlign(const MatrixIndexT rows,
                               const MatrixIndexT cols,
                               const MatrixStrideType stride_type) {
  if (rows * cols == 0) {
    KALDI_ASSERT(rows == 0 && cols == 0);
  ...
  Real tmp_val;
  int align_param = cblas_Xgemm_get_align(tmp_val);
  if (align_param < 16) align_param = 16;
  align_param = (align_param + 3) & (~3);

  // compute the size of skip and real cols
  skip = ((align_param / sizeof(Real)) - cols % (align_param / sizeof(Real)))
      % (align_param / sizeof(Real));
  stride = cols + skip;
  size = static_cast<size_t>(rows) * static_cast<size_t>(stride)
      * sizeof(Real);

  // allocate the memory and set the right dimensions and parameters
  if (NULL != (data = KALDI_MEMALIGN(align_param, size, &temp))) {
    MatrixBase<Real>::data_        = static_cast<Real *> (data);
    MatrixBase<Real>::num_rows_      = rows;
    MatrixBase<Real>::num_cols_      = cols;
    MatrixBase<Real>::stride_  = (stride_type == kDefaultStride ? stride : cols);
...
template<typename Real>
void Matrix<Real>::ResizeAlign(const MatrixIndexT rows,
                          const MatrixIndexT cols,
                          MatrixResizeType resize_type,
                          MatrixStrideType stride_type) {
  // the next block uses recursion to handle what we have to do if
  ...
  InitAlign(rows, cols, stride_type);
  if (resize_type == kSetZero) MatrixBase<Real>::SetZero();
}
#endif

template<typename Real>
void Matrix<Real>::ResizeRowsOnly(const MatrixIndexT r, const MatrixIndexT c) {
  KALDI_ASSERT(r > 0 && c > 0);
  MatrixIndexT skip, stride;
  size_t size;

  // compute the size of skip and real cols
  skip = ((16 / sizeof(Real)) - c % (16 / sizeof(Real)))
      % (16 / sizeof(Real));
  stride = c + skip;
  size = static_cast<size_t>(r) * static_cast<size_t>(stride) * sizeof(Real);
  if (size > MatrixBase<Real>::current_size_) {
    // KALDI_LOG << "size=" << size << ",current_size_=" << MatrixBase<Real>::current_size_;
    // size = size*2;
    if (MatrixBase<Real>::num_rows_ == 0) {
      size = size * 1.2;
    } else {
      double coeff = double(r+1) / (MatrixBase<Real>::num_rows_ + 1);
      size = coeff > 1.5 ? size*1.892 : coeff > 1.3 ? size*1.408 : coeff > 1.2 ? size*1.28 : size*1.2;
    }
    void *temp;
#if 1
    // this function is a special function for NnetEngine to save log likelihood
    // so we needn't to keep alignment. because there is no need to participate in matrix operations
    temp = realloc(MatrixBase<Real>::data_, size);
#else
    if (NULL != (temp = KALDI_MEMALIGN(16, size, &temp))) {
      // we need to free the data block if it was defined
      if (NULL != MatrixBase<Real>::data_) {
        std::memcpy(temp, MatrixBase<Real>::data_, MatrixBase<Real>::current_size_);
        KALDI_MEMALIGN_FREE( MatrixBase<Real>::data_);
      }
    }
#endif
    if (temp) MatrixBase<Real>::data_ = static_cast<Real *> (temp);
    MatrixBase<Real>::current_size_ = size;
  } else if (size*1.28 < MatrixBase<Real>::current_size_) {
    // reduce memory usage
    size = size*1.28;
    void* temp = realloc(MatrixBase<Real>::data_, size);
    if (temp) MatrixBase<Real>::data_ = static_cast<Real *> (temp);
    MatrixBase<Real>::current_size_ = size;
  }
  MatrixBase<Real>::num_rows_    = r;
  MatrixBase<Real>::num_cols_    = c;
  MatrixBase<Real>::stride_      = stride;
}
...
void MatrixBase<Real>::CopyFromMat(const MatrixBase<OtherReal> &M,
  ...
  this->stride_ = this->num_cols_;
  if (Trans == kNoTrans) {
    memcpy(static_cast<void *>(data_), static_cast<const void *>(M.Data()),
           num_rows_ * num_cols_ * sizeof(BaseQuantType));
  } else {
    KALDI_ASSERT(num_cols_ == M.NumRows() && num_rows_ == M.NumCols());
    int32 other_stride = M.NumCols();
    BaseQuantType *this_data = (BaseQuantType *)(data_);

    const BaseQuantType *other_data = (BaseQuantType *)(M.Data());
    for (MatrixIndexT i = 0; i < num_rows_; i++)
      for (MatrixIndexT j = 0; j < num_cols_; j++)
        this_data[i * num_cols_ + j] = other_data[j * other_stride + i];
  }

  this->quant_params_ = M.quant_params();
}
...
template<typename Real>
void Matrix<Real>::RemoveRows(MatrixIndexT from, MatrixIndexT to) {
  KALDI_ASSERT(static_cast<UnsignedMatrixIndexT>(from) <
               static_cast<UnsignedMatrixIndexT>(MatrixBase<Real>::num_rows_)
               && "Access out of matrix");
  KALDI_ASSERT(static_cast<UnsignedMatrixIndexT>(to) <
               static_cast<UnsignedMatrixIndexT>(MatrixBase<Real>::num_rows_)
               && "Access out of matrix");
  for (MatrixIndexT j = to + 1; j <  MatrixBase<Real>::num_rows_; j++)
    MatrixBase<Real>::Row(j-to+from-1).CopyFromVec( MatrixBase<Real>::Row(j));
  MatrixBase<Real>::num_rows_ -= (to-from+1);
}
...
void Matrix<Real>::Destroy() {
  ...
  MatrixBase<Real>::num_rows_ = MatrixBase<Real>::num_cols_
      = MatrixBase<Real>::stride_ = MatrixBase<Real>::current_size_ = 0;

  this->quant_params_.Reset();
  MatrixBase<Real>::data_type_ = REAL_DATA;
}
...
void MatrixBase<Real>::MulRowsVec(const VectorBase<Real> &scale) {
  ...
  if (QUANT_DATA != scale.VectorType() && QUANT_DATA != this->data_type_) {
  ...
  if (REAL_DATA == scale.VectorType() && QUANT_DATA == this->data_type_) {
    this->Dequantized();
    MatrixIndexT M = num_rows_, N = num_cols_;
  ...
  KALDI_ERR << "Vector's data type should be the same!";
}
...
void MatrixBase<Real>::MulColsVec(const VectorBase<Real> &scale) {
  ...
  if (REAL_DATA == this->data_type_ && REAL_DATA == scale.VectorType()) {
    for (MatrixIndexT i = 0; i < num_rows_; i++) {
      for (MatrixIndexT j = 0; j < num_cols_; j++) {
  ...
  KALDI_ERR << "error input data type " << this->data_type_ << "  "
            << scale.VectorType();
}
...
template <typename Real>
void MatrixBase<Real>::Write(std::ostream &os, bool binary, bool compress, bool quant) const {
  if (!os.good()) {
    KALDI_ERR << "Failed to write matrix to stream: stream not good";
  }

  std::vector<BaseQuantType> uint8_data;
  Real inner_min_val = -std::numeric_limits<float>::infinity(); // Min();
  Real inner_max_val = std::numeric_limits<float>::infinity(); // Max();
  if (quant) {
    int offset;
    if(this->num_cols_ * this->num_rows_ != 0) {
      inner_min_val = Min();
      inner_max_val = Max();
    }

    uint8_data.reserve(num_rows_ * num_cols_);
    Quantized(*this, uint8_data, inner_min_val, inner_max_val, offset);
  }
  ...
  if (binary) { // Use separate binary and text formats,
    ...
    WriteToken(os, binary, my_token);
    {
      ...
      if (quant) {
        WriteBasicType(os, binary, static_cast<BaseFloat>(inner_min_val));
        WriteBasicType(os, binary, static_cast<BaseFloat>(inner_max_val));
      }
    ...
    if (Stride() == NumCols()) {
      auto cp_size = quant ? sizeof(BaseQuantType) * static_cast<size_t>(num_rows_) * static_cast<size_t>(num_cols_)
                           : sizeof(Real) * static_cast<size_t>(num_rows_) * static_cast<size_t>(num_cols_);
      os.write(reinterpret_cast<const char *>(data_), cp_size);
    } else {
      if (!quant) {
        for (MatrixIndexT i = 0; i < num_rows_; i++)
        ...
        for (MatrixIndexT i = 0; i < num_rows_; i++)
          os.write(reinterpret_cast<const char *>(uint8_data.data() + i * this->stride_), sizeof(BaseQuantType) * num_cols_);
      }
    }
      ...
      for (MatrixIndexT i = 0; i < num_rows_; i++) {
        ...
        for (MatrixIndexT j = 0; j < num_cols_; j++)
          if (!quant) {
            ...
            os << static_cast<int32_t>(uint8_data[i * num_cols_ + j]) << " ";
          }
      }
      os << "]\n";
    }
...
template <typename Real>
void Matrix<Real>::Write(std::ostream &os, bool binary, bool compress, bool quant) const {
  if (!os.good()) {
    KALDI_ERR << "Failed to write matrix to stream: stream not good";
  }
  BaseFloat inner_min_val = -std::numeric_limits<float>::infinity(); // this->Min();
  BaseFloat inner_max_val = std::numeric_limits<float>::infinity(); // this->Max();
  std::vector<BaseQuantType> uint8_data;
  if (quant) {
    int offset;
    if(this->num_rows_ * this->num_cols_ != 0) {
      inner_min_val = this->Min();
      inner_max_val = this->Max();
    }
    uint8_data.reserve(this->num_rows_ * this->num_cols_);
    this->Quantized(*this, uint8_data, inner_min_val, inner_max_val, offset);
  }
  if (binary) { // Use separate binary and text formats,
    if (compress) {
      CompressedMatrix compressed_mat(*this);
      compressed_mat.Write(os, binary);
      return;
    }
    // since in binary mode we need to know if it's float or double.
    std::string my_token = (sizeof(Real) == 4 ? "FM" : "DM");
    ...
    WriteToken(os, binary, my_token);
    {
      ...
      if(quant) {
        os.write(reinterpret_cast<const char *>(&inner_min_val),
                sizeof(BaseFloat));
        os.write(reinterpret_cast<const char *>(&inner_max_val),
                sizeof(BaseFloat));
      }
    ...
    if (this->Stride() == this->NumCols()) {
      if (!quant) {
        os.write(reinterpret_cast<const char *>(this->data_),
                 sizeof(Real) * static_cast<size_t>(this->num_rows_) *
                     static_cast<size_t>(this->num_cols_));
      } else {
        os.write(reinterpret_cast<const char *>(uint8_data.data()),
                 sizeof(BaseQuantType) * static_cast<size_t>(this->num_rows_) *
                     static_cast<size_t>(this->num_cols_));
      }
    } else {
      if (!quant) {
        for (MatrixIndexT i = 0; i < this->num_rows_; i++)
          os.write(reinterpret_cast<const char *>(this->RowData(i)),
                   sizeof(Real) * this->num_cols_);
      } else {
        for (MatrixIndexT i = 0; i < this->num_rows_; i++)
          os.write(reinterpret_cast<const char *>(uint8_data.data() +
                                                  i * this->stride_),
                   sizeof(BaseQuantType) * this->num_cols_);
      }
    }
    if (!os.good()) {
      KALDI_ERR << "Failed to write matrix to stream";
    ...
    if (this->num_cols_ == 0) {
      ...
      for (MatrixIndexT i = 0; i < this->num_rows_; i++) {
        os << "\n  ";
        for (MatrixIndexT j = 0; j < this->num_cols_; j++)
          if (!quant) {
            os << (*this)(i, j) << " ";
          } else {
            os << static_cast<int32_t>(uint8_data[i * this->num_cols_ + j])
               << " ";
          }
      }
      os << "]\n";
...
void Matrix<Real>::Swap(Matrix<Real> *other) {
  ...
  std::swap(this->current_size_, other->current_size_);
}
...
void MatrixBase<Real>::AddVecToRows(const Real alpha, const VectorBase<OtherReal> &v) {
  if (REAL_DATA == this->data_type_ && REAL_DATA == v.VectorType()) {
  ...
  KALDI_ERR << "error data type in AddVecToRows  " << this->data_type_ << " "
            << v.VectorType();
}
