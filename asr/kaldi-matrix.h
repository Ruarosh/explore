...
typedef std::tuple<gemmlowp::OutputStageQuantizeDownInt32ByFixedPoint,
...
class MatrixBase {
 public:
  ...
  /// matrix type
  inline DataTypeT MatrixType() const { return data_type_; }
  ...
  /***************** quant functions ******************/
  void SetMatrixType(DataTypeT type);

  const QuantizationParams& quant_params() const { return this->quant_params_; }

  void Quantized();

  void Dequantized();

  void DequantizedAdd(int32_t* data, const QuantizationParams& params);

  void QuantizeMultiplierSmallerThanOne(float real_multiplier, int32_t* quantized_multiplier, int* right_shift) const;

  void OutputPipeline(const QuantizationParams& lhs_qparams,
                      const QuantizationParams& rhs_qparams,
                      QuantizationParams& mul_result,
                      OutputPipelineType& output_pipeline) const;

  void GetOutputRange(const BaseFloat input_min, const BaseFloat input_max,
                      BaseFloat* output_min, BaseFloat* output_max);

  void QuantizedAdd(const BaseQuantType* input_mat, bool to_dequantize,
                    const QuantizationParams* quant_param);

  void QuantizedTensorInt32ToUint8(const int32_t* input_data,
                                   bool to_dequantize, const BaseFloat min_val,
                                   const BaseFloat max_val);

  void ChooseQuantizationParams(const BaseFloat min_val,
                                const BaseFloat max_val,
  ...
  */
  ...
  void AddMatMatReal(const Real alpha,
                 const MatrixBase<Real>& A, MatrixTransposeType transA,
                 const MatrixBase<Real>& B, MatrixTransposeType transB,
                 const Real beta);

  void AddMatMatL8R8ToRst32_2(const Real alpha,
                 const MatrixBase<Real>& A, MatrixTransposeType transA,
                 const MatrixBase<Real>& B, MatrixTransposeType transB,
                 const Real beta, bool to_dequantize);

  void AddMatMatL8R8ToRst32(const Real alpha,
                 const MatrixBase<Real>& A, MatrixTransposeType transA,
                 const MatrixBase<Real>& B, MatrixTransposeType transB,
                 const Real beta, bool to_dequantize);

  void AddMatMatL8R8ToRst8(const Real alpha,
                 const MatrixBase<Real>& A, MatrixTransposeType transA,
                 const MatrixBase<Real>& B, MatrixTransposeType transB,
                 const Real beta, bool to_dequantize);

  bool PreAddMatMat(const Real alpha,
                 const MatrixBase<Real>& A, MatrixTransposeType transA,
                 const MatrixBase<Real>& B, MatrixTransposeType transB,
                 const Real beta, bool to_dequantize);
  ...
  void AddMatMat(const Real alpha,
                 const MatrixBase<Real>& A, MatrixTransposeType transA,
                 const MatrixBase<Real>& B, MatrixTransposeType transB,
                 const Real beta, bool to_dequantize = true);

#ifdef HAVE_BLAS_PRECOPY
  void MatPrecopy(MatrixTransposeType transA, MatrixTransposeType transB, const MatrixBase<Real>& B);

  void AddMatMatPrecopy(const Real alpha,
                 const MatrixBase<Real>& A, MatrixTransposeType transA,
                 const MatrixBase<Real>& B, MatrixTransposeType transB,
                 const Real beta);
#endif
  ...
  /// write to stream.
  void Write(std::ostream & out, bool binary, bool compress = false, bool quant = false) const;
 ...
 protected:
  ...
  explicit MatrixBase(Real *data, MatrixIndexT cols, MatrixIndexT rows, MatrixIndexT stride) :
    data_(data), data_type_(REAL_DATA), num_cols_(cols), num_rows_(rows), stride_(stride) {
    // KALDI_ASSERT_IS_FLOATING_TYPE(Real);
  }

  ///  Initializer, callable only from child.
  /// Empty initializer, for un-initialized matrix.
  explicit MatrixBase() : data_(NULL), data_type_(REAL_DATA) {
    // KALDI_ASSERT_IS_FLOATING_TYPE(Real);
  }
...
private:
  ...
  double QuantizeScaleAdd(Real range_min, Real range_max) const;

  void Quantized(const MatrixBase<Real>& src_mat,
                 std::vector<BaseQuantType>& dst_mat, const Real min_val,
                 const Real max_val, int& offset) const;

  void RealTensorToQuantizedInPlace(BaseQuantType* dst, QuantizationParams& quant_param) const;

  template <typename InputType>
  BaseQuantType RealToQuantized(Real input, Real range_min, double scale) const;

  template <typename InputType>
  int64_t RealToQuantizedOffset(Real input, Real range_min, double range_scale) const;

  Real RealForOneQuantizedLevel(Real range_min, Real range_max) const;

  void QuantizationRangeForMultiplication(Real min_a, Real max_a, Real min_b,
                                          Real max_b, BaseFloat& min_output,
                                          BaseFloat& max_output) const;

  template <typename InputType>
  Real QuantizedToReal(InputType input, Real range_min, double range_scale);

  void GemmlowpMultiplyL8R8ToRst32(gemmlowp::GemmContext* context,
                        const BaseQuantType* a_data,
                        const MatrixTransposeType A_order,
                        const BaseQuantType* b_data,
                        const MatrixTransposeType B_order, int depth,
                        int offset_a, int offset_b, int lda, int ldb, int ldc,
                        BaseQuantType* output_data);

  void GemmlowpMultiplyL8R8ToRst8(gemmlowp::GemmContext* context,
                        const BaseQuantType* a_data,
                        const MatrixTransposeType A_order,
                        const BaseQuantType* b_data,
                        const MatrixTransposeType B_order, int depth,
  ...
  template <typename InputType>
  double DequantizeScale(const BaseFloat min_val, const BaseFloat max_val) const;
  /***************** quant functions ******************/
...
class Matrix : public MatrixBase<Real> {
 public:
  ...
  void Read(std::istream & in, bool binary, bool add = false);
  /// read quantized matrix
  void ReadQuantizedData(std::istream& is, bool binary);
  /// write to stream.
  void Write(std::ostream & out, bool binary, bool compress = false, bool quant = false) const;
  ...
  /// Remove some specified row range.
  // [from, to] will be removed
  void RemoveRows(MatrixIndexT from, MatrixIndexT to);
...
#ifdef HAVE_BLAS_PRECOPY
  void ResizeAlign(const MatrixIndexT r,
            const MatrixIndexT c,
            MatrixResizeType resize_type = kSetZero,
            MatrixStrideType stride_type = kDefaultStride);
#endif

  void ResizeRowsOnly(const MatrixIndexT r, const MatrixIndexT c);
...
#ifdef HAVE_BLAS_PRECOPY
  void InitAlign(const MatrixIndexT r,
            const MatrixIndexT c,
            const MatrixStrideType stride_type);
#endif
